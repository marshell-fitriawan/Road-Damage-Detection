...
def generate_frames():
    ...
    while True:
        ...
        if camera_source != active_source:
            ...
            cap = open_capture(active_source)
            # Reset window
            frame_count = 0; best_conf = 0.0
            best_frame = None; best_annotated = None

        success, frame = cap.read()
        if not success:
            frame = np.zeros((480, 640, 3), dtype="uint8")
            cv2.putText(frame, "Tidak dapat membaca kamera", (60, 240),
        ...
        if frame_count >= FRAME_WINDOW:
            if best_frame is not None:
                last_saved = save_best_frame(best_frame, best_annotated, best_conf, best_class)
        ...
        if last_saved:
            cv2.putText(annotated, f"Tersimpan: {last_saved}", (8, annotated.shape[0] - 10),
...
def saved_frames_page():
    ...
    rows = "".join(
        f'<div class="sf-item">'
        f'<img src="/saved_frames/{f}" alt="{f}">'
        f'<p>{f}</p>'
        f'</div>'
        for f in files
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        ssl_context = None
        proto = 'http'
        print("⚠️  pyOpenSSL tidak terinstall — kamera HP via browser tidak bisa akses kamera.")
    ...
    if ssl_context:
        ...
        print("  (Terima peringatan sertifikat di browser HP — klik 'Advanced' lalu 'Proceed')")
