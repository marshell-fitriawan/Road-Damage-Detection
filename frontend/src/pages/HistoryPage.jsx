...
const HistoryPage = () => {
  ...
  const handleBulkDelete = async () => {
    ...
    if (
      !confirm(
        `Hapus ${selectedIds.size} data kerusakan yang dipilih? Tindakan ini tidak bisa dibatalkan.`,
      )
    )
      return;
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-400 mt-1">
                Lihat, periksa, ubah, atau hapus data kerusakan jalan
              </p>
            </div>
          ...
          {loading ? (
            ...
            <div className="card text-center py-12">
              <p className="text-gray-400 text-lg">
                Belum ada data kerusakan jalan
              </p>
            </div>
          ) : (
                  ...
                  >
                    ...
                    {damage.image_path && (
                      <div
                        ...
                        onClick={
                          selectMode ? () => toggleSelect(damage.id) : undefined
                        }
                      >
                        <img
                    ...
                    <div
                      className={`inline-block px-3 py-1 rounded-full text-sm font-semibold mb-3 ${getDamageTypeColor(damage.damage_type)}`}
                    >
                      {damage.damage_type}
                    </div>
                    <div className="space-y-2 mb-4">
                      <div className="flex items-center justify-between">
                        ...
                        <span
                          className={`px-2 py-1 rounded text-xs font-semibold ${getSeverityBadge(damage.severity)}`}
                        >
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <select
                          value={damage.status}
                          onChange={(e) =>
                            handleStatusUpdate(damage.id, e.target.value)
                          }
                      ...
                      {damage.latitude && damage.longitude && (
                        <div className="flex items-center gap-2 text-sm text-gray-400">
                          ...
                          <span className="font-mono text-xs">
                            {damage.latitude.toFixed(4)},{" "}
                            {damage.longitude.toFixed(4)}
                          </span>
                        </div>
                      )}
                      {damage.tracking_session &&
            ...
            >
              <div className="card max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                {selectedDamage.image_path && (
                  <img
                    src={`/storage/${selectedDamage.image_path}`}
                    alt={selectedDamage.damage_type}
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-bold">{selectedDamage.damage_type}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-400">Confidence</p>
                      <p className="font-bold">
                        {(selectedDamage.confidence * 100).toFixed(1)}%
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-400">Keparahan</p>
                  ...
                  <div>
                    ...
                    />
                    <button
                      onClick={() => handleSaveNotes(selectedDamage.id)}
