...
import {
  LayoutDashboard, Map, Camera, History, Users, Route as RouteIcon,
  Menu, X, LogOut, Play
} from 'lucide-react';
    ...
    <>
      ...
      <aside className="hidden lg:flex lg:flex-col lg:w-64 bg-accent border-r border-gray-700 h-screen sticky top-0">
        ...
        <div className="px-4 py-3 border-b border-gray-700">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-400 capitalize">{user.role === 'admin' ? 'Administrator' : 'Petugas Lapangan'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-gray-700">
          <div className="bg-secondary rounded-lg p-3">
            ...
            <p className="text-xs text-gray-400 mt-1">YOLOv8 + Laravel + React</p>
          </div>
        </div>
      </aside>
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 z-40 bg-secondary">
          <div className="p-6 border-b border-gray-700">
            <h1 className="text-xl font-bold text-primary">Road Damage Detection</h1>
            <p className="text-sm text-gray-400 mt-1">{user.name} - {user.role === 'admin' ? 'Administrator' : 'Petugas'}</p>
          </div>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${
                    isActive(item.path) ? 'bg-primary text-white' : 'text-gray-300 hover:bg-accent'
                  }`}
                >
                  <Icon className="w-5 h-5" />
      ...
      <main className="flex-1 p-4 lg:p-8 overflow-auto">
        <div className="max-w-7xl mx-auto">
          <Routes>
            ...
            <Route path="/" element={
              user ? (
                user.role === 'admin' ? <Navigate to="/admin/dashboard" replace /> : <Navigate to="/petugas/dashboard" replace />
            ...
            <Route path="/admin/peta" element={
              <ProtectedRoute roles={['admin']}><MapPage /></ProtectedRoute>
            } />
            <Route path="/admin/kerusakan" element={
              <ProtectedRoute roles={['admin']}><HistoryPage /></ProtectedRoute>
            } />
            <Route path="/admin/tracking" element={
              <ProtectedRoute roles={['admin']}><TrackingHistoryPage showAll={true} /></ProtectedRoute>
            } />
            ...
            <Route path="/petugas/dashboard" element={
              <ProtectedRoute roles={['petugas']}><PetugasDashboard /></ProtectedRoute>
            } />
            <Route path="/petugas/tracking" element={
              <ProtectedRoute roles={['petugas']}><TrackingPage /></ProtectedRoute>
            } />
            <Route path="/petugas/peta" element={
              <ProtectedRoute roles={['petugas']}><MapPage /></ProtectedRoute>
            } />
            <Route path="/petugas/riwayat" element={
              <ProtectedRoute roles={['petugas']}><TrackingHistoryPage showAll={false} /></ProtectedRoute>
            } />
